...
export function ChatBoxPropsPanel({
  w,
  upW,
}: {
  w: Extract<OverlayWidget, { kind: "chatBox" }>;
      ...
      <PropSection title="Chat Box">
        ...
        <Field label={`Padding ${chatPadding}px`}>
          <input
            ...
            value={chatPadding}
            onChange={(e) =>
              upW((v) => v.kind === "chatBox" ? { ...v, data: { ...v.data, chatPadding: Number(e.target.value) } } : v)
            }
          />
        </Field>
        <label className="flex items-center gap-2 cursor-pointer text-xs text-[var(--text-secondary)]">
          <input
            ...
            checked={showFrame}
            onChange={(e) =>
              upW((v) => v.kind === "chatBox" ? { ...v, data: { ...v.data, showFrame: e.target.checked } } : v)
            }
          />
          Mostrar frame
        </label>
          ...
          <>
            <Field label="Título">
              <input
                ...
                value={frameTitle}
                onChange={(e) =>
                  upW((v) => v.kind === "chatBox" ? { ...v, data: { ...v.data, frameTitle: e.target.value } } : v)
                }
              />
            </Field>
        ...
        <CustomWidgetEditorModal
          widget={{
            ...
            data: {
              ...
              propsJson: JSON.stringify({
                msg: { username: "ChatUser", message: "Hola mundo! 🎉", color: "#ff9a5c", timestamp: new Date().toISOString() },
              }, null, 2),
            },
          } as unknown as Extract<OverlayWidget, { kind: "custom" }>}
